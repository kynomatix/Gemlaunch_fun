...
## October 11, 2025 - IPFS Integration: Permanent Image Storage Implemented ✅
- **Problem Identified**: Replicate URLs expire after 24-48 hours
- **Solution Implemented**: IPFS via Pinata (Task 2.11)
  - All token images now stored permanently on IPFS
  - Images generated by AI, immediately uploaded to Pinata IPFS
  - IPFS URL format: `ipfs://QmXg7...` (permanent content-addressed storage)
  - Gateway URL for display: `https://gateway.pinata.cloud/ipfs/{hash}`
  - PINATA_JWT secret configured and verified
- **Implementation Details**:
  - Added `upload_to_ipfs()` function in `services/image_generator.py`
  - Downloads Replicate image and uploads to Pinata automatically
  - Returns both IPFS URL (database storage) and gateway URL (frontend display)
  - Timeout handling: 60s download, 120s upload
...
- **Benefits**:
  ...
  - Free tier: 1GB storage on Pinata
