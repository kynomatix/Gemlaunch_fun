...
## October 11, 2025 - IPFS Integration: Planning Complete (Task 2.11)
- **Problem Identified**: Replicate URLs expire after 24-48 hours
- **Solution Planned**: IPFS via Pinata
  - PINATA_JWT secret configured ✅
  - `upload_to_ipfs()` helper function created ✅
  
- **Correct Implementation Flow** (to be implemented):
  1. **Image Generation**: User generates multiple images with Replicate (temporary URLs)
  2. **User Selection**: User picks their favorite image
  3. **Token Deployment**: When user confirms token creation:
     - Upload selected image to IPFS via Pinata
     - Get permanent `ipfs://` URL
     - Store IPFS URL in database with deployed token
  4. **Display**: Frontend shows image via gateway URL
  
- **Why This Flow?**:
  - User might generate 4+ images before finding the right one
  - Only upload the final selected image (saves IPFS storage)
  - IPFS upload happens as part of token deployment transaction
  - Ensures only deployed tokens have permanent images
  
- **Benefits**:
  - Images never expire (permanent decentralized storage)
  - Content-addressed (same image = same hash)
  - Blockchain-ready for future on-chain metadata
  - Efficient storage usage (only final images uploaded)
