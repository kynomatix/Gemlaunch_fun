#!/usr/bin/env python3
"""
Update TokenFactory V11 to point to GraduationController V12
...
def update_tokenfactory_gc():
    """Update TokenFactory V11 to use GraduationController V12"""
    
    web3_service = get_web3_service()
    w3 = web3_service.w3
    
    print("=" * 80)
    print("🔧 UPDATING TOKENFACTORY V11 TO USE GRADUATIONCONTROLLER V12")
    print("=" * 80)
    
    TOKEN_FACTORY_V11 = "0x427B039bc381911a40AC25Fc50AB9e6f5633A5B1"
    GRADUATION_CONTROLLER_V12 = "0xD7B75104f005DFC9dE004fdb97399444752d66D3"
    
    print(f"\n📋 Configuration:")
    print(f"   TokenFactory V11: {TOKEN_FACTORY_V11}")
    print(f"   New GC V12 (COMPLETE FIX): {GRADUATION_CONTROLLER_V12}")
    ...
    if current_gc.lower() == GRADUATION_CONTROLLER_V12.lower():
        print(f"✅ Already pointing to V12! No update needed.")
        return True
    
    print(f"   Updating from: {current_gc}")
    print(f"   Updating to:   {GRADUATION_CONTROLLER_V12}")
    ...
    tx = tf.functions.setGraduationController(
        Web3.to_checksum_address(GRADUATION_CONTROLLER_V12)
    ).build_transaction({
        'from': owner_address,
        'nonce': nonce,
        'gas': 100000,
    ...
    try:
        ...
        if receipt['status'] == 1:
            ...
            if new_gc.lower() == GRADUATION_CONTROLLER_V12.lower():
                print(f"   ✅ VERIFIED: TokenFactory V11 now points to GraduationController V12!")
                print(f"   ✅ COMPLETE STF FIX ACTIVE: IERC721Receiver + unsafe burn transfer")
            else:
            ...
            print(f"   4. CREATE A NEW TOKEN (V11 tokens like KYR cannot be fixed)")
            print(f"   5. Test graduation with the new token at $50")
            print(f"\n⚠️  V11/V10 tokens will still fail - they have old GC embedded")
            print(f"   Only NEW tokens will use V12 and graduate successfully")
            print(f"\n" + "=" * 80)
...
if __name__ == "__main__":
    ...
    if success:
        print(f"\n✅ TokenFactory V11 updated to use GraduationController V12")
    else:
        print(f"\n❌ Update failed")
        sys.exit(1)
