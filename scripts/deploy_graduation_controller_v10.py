#!/usr/bin/env python3
"""
Deploy GraduationController V10 - STF FIX VERSION
Fixes token approval ordering to prevent "execution reverted: STF" errors
"""
...
def deploy_graduation_controller_v10():
    """Deploy GraduationController V10 with STF fix"""
    
    web3_service = get_web3_service()
    w3 = web3_service.w3
    
    print("=" * 80)
    print("🚀 DEPLOYING GRADUATIONCONTROLLER V10 (STF FIX)")
    print("=" * 80)
    print("\n🔧 FIX: Token approvals now happen BEFORE pool creation")
    print("   This prevents STF errors during createAndInitializePoolIfNecessary()")
    ...
    print(f"   Internal Version: 3.0.0 (Deployed as V10)")
    ...
    try:
        ...
        if receipt['status'] == 1:
            ...
            deployment_info = {
                ...
                "version": "10.0.0",
                "fixes": "Token approval ordering - prevents STF errors",
                "constructorParams": {
            ...
            output_file = 'deployments/graduation_controller_v10.json'
            with open(output_file, 'w') as f:
                json.dump(deployment_info, f, indent=2)
            
            print(f"\n💾 Deployment info saved to: {output_file}")
            print(f"\n" + "=" * 80)
            print(f"🎉 GRADUATIONCONTROLLER V10 DEPLOYED SUCCESSFULLY!")
            print(f"=" * 80)
            print(f"\n📝 NEXT STEPS:")
            print(f"   1. Update contracts/deployed_addresses.json")
            print(f"   2. Update services/web3_service.py with new address")
            print(f"   3. Run validation script to verify configuration")
            print(f"   4. Test graduation with a new $50 token")
            print(f"\n" + "=" * 80)
...
if __name__ == "__main__":
    address = deploy_graduation_controller_v10()
    if address:
        print(f"\n✅ New GraduationController V10: {address}")
    else:
        print(f"\n❌ Deployment failed")
        sys.exit(1)
