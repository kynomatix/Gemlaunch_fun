#!/usr/bin/env python3
"""
Update TokenFactory V11 to point to GraduationController V10
...
def update_tokenfactory_gc():
    """Update TokenFactory V11 to use GraduationController V10"""
    
    web3_service = get_web3_service()
    w3 = web3_service.w3
    
    print("=" * 80)
    print("🔧 UPDATING TOKENFACTORY V11 TO USE GRADUATIONCONTROLLER V10")
    print("=" * 80)
    
    TOKEN_FACTORY_V11 = "0x427B039bc381911a40AC25Fc50AB9e6f5633A5B1"
    GRADUATION_CONTROLLER_V10 = "0x7384F95729Ff5c2B2BFe4Cc101139a13A85a66e9"
    
    print(f"\n📋 Configuration:")
    print(f"   TokenFactory V11: {TOKEN_FACTORY_V11}")
    print(f"   New GC V10: {GRADUATION_CONTROLLER_V10}")
    ...
    if current_gc.lower() == GRADUATION_CONTROLLER_V10.lower():
        print(f"✅ Already pointing to V10! No update needed.")
        return True
    
    print(f"   Updating from: {current_gc}")
    print(f"   Updating to:   {GRADUATION_CONTROLLER_V10}")
    ...
    tx = tf.functions.setGraduationController(
        Web3.to_checksum_address(GRADUATION_CONTROLLER_V10)
    ).build_transaction({
        'from': owner_address,
        'nonce': nonce,
        'gas': 100000,
    ...
    try:
        ...
        if receipt['status'] == 1:
            ...
            if new_gc.lower() == GRADUATION_CONTROLLER_V10.lower():
                print(f"   ✅ VERIFIED: TokenFactory V11 now points to GraduationController V10!")
            else:
            ...
            print(f"=" * 80)
            print(f"\n📝 NEXT STEPS:")
            print(f"   1. Restart application to pick up changes")
            print(f"   2. Test graduation with a new $50 token")
            print(f"\n" + "=" * 80)
...
if __name__ == "__main__":
    ...
    if success:
        print(f"\n✅ TokenFactory V11 updated to use GraduationController V10")
    else:
        print(f"\n❌ Update failed")
        sys.exit(1)
